...
export const $ButtonPrimary = (config: IButton) => {
  return $Button({
    ...config,
    buttonOp: O(
      style({ color: 'white', whiteSpace: 'nowrap', fill: 'white', background: 'linear-gradient(45deg,  #8A5FCF 21%, #D298ED 100%)', borderRadius: '30px', padding: '12px 24px', fontWeight: 'bold', borderColor: pallete.background }),
      config.buttonOp || O()
    )
  })
}
